...
import SchoolDashboardClient from "@/app/dashboard/school/SchoolDashboardClient";

const JWT_SECRET = new TextEncoder().encode(process.env.SUPABASE_SERVICE_ROLE_KEY || "fallback_secret_key");

export default async function TopLevelSchoolDashboardPage() {
  ...
  if (hasSupabaseAdminConfig) {
    let sData = null;
    if (schoolId) {
      const { data } = await (supabaseAdmin as any).from("schools").select("*").eq("id", schoolId).maybeSingle();
      sData = data;
    }
    if (!sData && schoolCode) {
      const { data } = await (supabaseAdmin as any).from("schools").select("*").ilike("school_code", schoolCode.trim()).maybeSingle();
      sData = data;
    }
